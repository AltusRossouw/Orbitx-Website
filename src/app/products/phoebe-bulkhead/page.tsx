...
export default function PhoebeBulkheadProductPage() {
  const phoebe = products.find(p => p.id === 'phoebe-bulkhead')
  const image = phoebe?.image || '/images/products/phoebe-bulkhead-800x418.png'
  const specPdf = phoebe?.pdfSpec || '/pdfs/OrbitX-Phoebe-bulkhead-specifications.pdf'

  // IES files for Phoebe Bulkhead
  const IES_BASE = '/products/phoebe-bulkhead/docs/'
  const iesFiles: Record<string, string> = {
    '280-15-black': 'OrbitX Phoebe 15W.ies',
    '280-15-white': 'OrbitX Phoebe 15W.ies',
  }
  ...
  return (
    <div className="min-h-screen bg-black text-white flex flex-col">
      ...
      <main className="flex-1">
        {/* Hero */}
        <section id="phoebe-bulkhead" className="relative py-16 md:py-20 bg-gray-900 overflow-hidden">
          ...
          <div className="relative z-10 container mx-auto px-6">
            <div className="grid lg:grid-cols-2 gap-10 items-center">
              <div>
                ...
                <p className="text-orbitx-accent text-sm font-medium mb-3">IP65 INTEGRATED SURFACE MOUNT LED LIGHT</p>
                <motion.p 
                ...
                >
                  OrbitX Direct Drive Technology onboard. Available in white and black. IP65 rated for outdoor and wet locations with Type A light quality.
                </motion.p>
              ...
              >
                <Image src={image} alt="Phoebe Bulkhead" fill className="object-contain p-6" />
              </motion.div>
            </div>
          </div>
        </section>
          ...
          <div className="container mx-auto px-6 grid lg:grid-cols-2 gap-10">
            <div className="space-y-6">
              <SpecCard title="Lifetime & Guarantee" items={[
                'Nominal Lifetime (L70): 70,000 hours (IES LM-80, TM-21)',
            ...
            <div className="space-y-6">
              ...
              <SpecCard title="Interference Compliance" items={[
                'WiFi/Barcode/Radio: To International Telecommunications Center, Inc.',
                'THD: IEC 61000-3-2 Class D Compliant'
              ]} />

              <SpecCard title="Additional Options" items={[
                'Battery Backup: 1000lm for 4h, 6h charge time, 1y guarantee',
                'Radar Motion Sensor: Programmable sensitivity & time on',
                'Day/Night Sensor: Auto on/off at dusk/dawn',
          ...
          <div className="container mx-auto px-6">
            ...
            <div className="relative rounded-xl border border-gray-800 bg-black overflow-hidden">
              <div className="relative w-full h-72 md:h-96">
                <Image
                  src="/products/phoebe-bulkhead/images/phoebe-bulkhead-dimensions.png"
                  alt="Phoebe Bulkhead dimensions"
                  fill
              ...
              <table className="min-w-full bg-gray-900 text-sm">
                <thead className="bg-gray-800 text-gray-300">
                  <tr>
                    ...
                    <th className="text-center p-3 font-semibold">Housing Color</th>
                ...
                <tbody className="divide-y divide-gray-800 text-gray-300">
                  {[
                    ['OXPH280155B', 'Black', '15 W', '1', '290 × 290 × 90', '1.4', '280-15-black'],
                    ['OXPH280155W', 'White', '15 W', '1', '290 × 290 × 90', '1.4', '280-15-white'],
                  ].map((row, i) => {
                    const iesKey = row[6] as string
...
// ---------------- Graph Components (Phoebe specific) ----------------
...
// 1) Light Survival Rate Curve (70k hour lifetime)
function PhoebeSurvivalCurve() {
  const pts = [
    [0,100],[5,100],[10,100],[15,100],[20,100],[25,100],[30,100],[35,100],[40,100],[45,100],
    [50,100],[55,100],[60,100],[65,100],[70,99.5],[75,98],[80,95],[85,91],[90,86],[95,80],[100,72]
  ].map(d => ({ x: d[0], y: d[1] }))
...
// 2) Lumen Maintenance (L70) (70k hour L70)
function PhoebeLumenMaintenance() {
  const pts = [
    [0,100],[10,98],[20,96],[30,94],[40,92],[50,90],[60,87],[70,83],[75,80],[80,75],[85,70],[90,65],[95,58],[100,50]
  ].map(d => ({ x: d[0], y: d[1] }))
  const W = 520, H = 300, pad = 44
  const xMin = 0, xMax = 100, yMin = 0, yMax = 100
  const x = (v: number) => scale(v, xMin, xMax, pad, W - pad)
  ...
  const l70y = y(70)
  const l70x = x(70) // 70k hours for L70
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto">
      ...
      <text x={l70x + 6} y={y(8)} fontSize={11} fill="#9aa0a6">~70k h</text>
      <text x={x(8)} y={l70y - 6} fontSize={12} fill="#ff4d4f">L70</text>
    </svg>
  )
}

// 3) CRI vs GAI (same Type A values for Phoebe)
function CRIvsGAI() {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto">
      ...
      <defs>
        <marker id="arrowBluePhoebe" markerWidth="10" markerHeight="10" refX="8" refY="5" orient="auto">
      ...
      {(() => {
        ...
        return (
          <>
            ...
            {/* Arrow to red ring */}
            <path d={`M ${arrowStartX},${arrowStartY} L ${arrowEndX},${arrowEndY}`} stroke="#2563eb" strokeWidth={2.5} strokeDasharray="6 6" strokeLinecap="round" fill="none" markerEnd="url(#arrowBluePhoebe)" />
          </>
        )
      })()}
    </svg>
...
// 4) Spectral Power Distribution (same as other products)
function SPDPhoebe() {
  const data = [
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto">
      <defs>
        <linearGradient id="spdGradientPhoebe" x1={pad} x2={W - pad} y1={0} y2={0} gradientUnits="userSpaceOnUse">
      ...
      <path d={areaPath} fill="url(#spdGradientPhoebe)" opacity={0.95} />
